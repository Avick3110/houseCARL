...
- **Don't append a new domain to a large existing file.** A new subsystem or tool family's service logic lands as its own file/service; the facade may keep a thin delegating member, but the logic gets its own home. `LoadOrderService.cs` is the standing example of the accretion this rule stops — nothing ever asked "does this belong in this file?", so code landed where the last thing went. (Adopted 2026-08-19 from the code-quality review; the carve-up of already-accreted domains is chartered post-2.0 — `dev/plans/STRUCTURAL_SEAM_SPLIT_PLAN_2026-08-19.md`.)
