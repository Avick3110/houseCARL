...
11. **Pre-PR review rounds — the branch is reviewed before the PR opens, with fresh independent agents, bounded by a convergence rule.** Before pushing a code branch: spawn independent review agents over the branch diff, fold what survives triage, then spawn a **new** round. Stopping is governed by these rules, in order:
