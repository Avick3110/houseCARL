...
- [6. The embedded `.nif` texture references — where the mesh-side material lives](#6-embedded-nif-references)
...
houseCARL** (it reads no tint/skin pixels). The `.nif` is not: `housecarl_nif_inspect` reads its
...
- **Plugin load order** decides which mod's **NPC record** wins — what `housecarl_records` returns.
...
(`asset_status`) alongside the record winner (`housecarl_records`) is houseCARL's structural advantage.
...
notorious for this"). **`housecarl_nif_inspect` reads those shape names and the embedded tint/skin paths**,
so mode ii is *checkable* — compare the mesh's baked names against the record's HeadParts.
...
(`housecarl_records` / `housecarl_apply` / `housecarl_forward`) · **houseCARL-nif** (`nif_inspect` reads the `.nif`'s data
...
The embedded-value edits — texture path, material/`.tri`/xml ref, shape name, flags/alpha/partition — are
houseCARL's at the data layer; only pixels and geometry are instructed.

| # | Cause | Mechanism | houseCARL tell | Fix class |
|---|---|---|---|---|
| **A** | **Record-winner vs file-winner desync** (dominant) | Load order picks record B; VFS picks mod A's facegen file → file built from a different mod's appearance than the winning record | `housecarl_records` winner ≠ `asset_status` winner on the `.nif`/`.dds` pair | houseCARL-file and/or -record |
...
| **C** | **Non-appearance edit reverts head parts** | A later non-facegen plugin (USSEP-style) wins the record and reverts head parts toward vanilla while overhaul A's file still wins | `housecarl_records` shows the non-appearance plugin winning head parts; `asset_status` shows A's file still winning | houseCARL-record (often + -file) |
...
| **H** | **Head-part / hair / brow conflict or missing HDPT** | Record references an HDPT whose mesh is absent/replaced, or whose internal shape names don't match | partial dark/clipping; `housecarl_records` head-parts list vs `nif_inspect` shape names; mesh winner ≠ expected | split: houseCARL-record / houseCARL-nif (`rename_shape`) / CK |
| **I** | **Face-vs-body texture mismatch** (unique-bodies-by-race) | Head `.nif` hardcodes the vanilla skin path; a per-race body framework gives a unique body but the head still loads vanilla → head/body color mismatch. **NOT the dark-face bug.** | detect the *scenario*, and `nif_inspect` READS the `.nif` skin slots (0/1) so you see the wrong path; `nif_set set_path` at slot 0/1 REWRITES it (the NPC Facegen Patcher edit) | houseCARL-nif (`set_path` slots 0/1) |
| **J** | **Skin-tone / TextureLighting / HeadTexture record mismatch** | Winner's TintLayers / TextureLighting (QNAM) / HeadTexture / HairColor inconsistent with body race/skin | wrong-colored but correct-shaped head; `housecarl_records` these fields vs intended source | houseCARL-record (pair with re-bake/FDF) |
| **K** | **CK baked against the wrong load order/masters** | Facegen baked while the wrong override resolved (missing master) → right filename, wrong content | wrong (not dark) face; `housecarl_records` shows the true winner to bake against | CK-instructed |
| **L** | **Missing master / orphan override** | Override copied without its master (or master disabled) → record can't resolve | dark/missing actor, but `asset_status` shows the correct file winning; `housecarl_records` masters | houseCARL-record (masters-by-construction) / report |
| **M** | **Missing appearance dependency** (High Poly Head, KS Hairdos) | A required resource mod is disabled while the record still references it | headless/floating parts; `housecarl_records` head-parts links unresolved, or asset absent | runtime-mod / report (install) |
...
> repointed at the destination — **`nif_set set_path texture_slot=6`**, verified before landing, rather than a
> NifSkope escalation. A bulk run across many such NPCs is still faster in
> FaceGenEslify.
...
For head/body color mismatch that is a *record* problem (Cause J): a `housecarl_apply` op on TextureLighting (QNAM
...
NifSkope, but a plain diffuse/normal/tint slot swap is houseCARL's. Editing TintLayers/morph WITHOUT
...
the old FormID*, and `nif_set set_path texture_slot=6` REWRITES it (verified before landing). Both halves sit
at the data layer.
**FaceGenEslify automates only the rename — its embedded-path edit is a MANUAL NifSkope step (its own
README step 9); houseCARL performs that manual step itself via `set_path`, so don't route the user to
...
toward a master/resolution problem — confirm via `housecarl_records` before reaching for a file fix (Fix B will
...
| **Neck seam, face correctly colored but ≠ body** | I or J | I = `.nif` hardcodes vanilla skin — `nif_inspect` shows the slot-0/1 skin path and `nif_set set_path` rewrites it (the NPC Facegen Patcher edit, at the data layer). J = record QNAM/TintLayers inconsistent (houseCARL-record). |
...
## 6. Embedded `.nif` texture references

The head `.nif` carries its own asset references — the per-NPC FaceTint `.dds` at binary texture slot 6,
...
- **The front door is the first failure point.** Resolve via EditorID (best) or name (disambiguate).
  `housecarl_records` and `housecarl_nif_inspect` `npc=` both READ the runtime form the console and the
  logs print (`FExxxYYY` / `XX######`), resolved against the current load order, and the response names
...
houseCARL drives the *file*, *record* and whitelisted *`.nif`-value* halves itself — the
...
| **Dark Face Issue Reporter** (Nexus 42133) | xEdit script; enumerates every NPC from a chosen master, checks each record's HeadParts against the resolvable NIF, flags `DarkFaceIssue`; emits in-game `prid`/`moveto` batch files. Does NOT auto-fix. | Whole-mod / load-order batch diagnosis. The same comparison at BOTH layers: the VFS winner (`asset_status`) *and* the record-HeadParts-vs-baked-NIF-names check (`nif_inspect`). A whole-order sweep is one `housecarl_records` call spilled with `to_file=` — see §8. |
...
- **Mode ii is NAME/PATH-detectable *and* repairable, but not render-verifiable.** `nif_inspect` reads
  the `.nif`'s baked shape names and embedded tint/skin paths, so a *mismatch* against the record's HeadParts
  is a confirmed mode-ii finding, and `nif_set` (`rename_shape` / `set_path` /
  `set_flags` / `set_alpha`) *repairs* the name/path/flag class it finds, verified before landing. The
...
- **A console click identifies a placed reference, not the base actor.** The runtime form itself resolves
  on the read tools; confirming that the record it resolved to is the NPC the user means is the caller's
  step, and it is not skippable.
