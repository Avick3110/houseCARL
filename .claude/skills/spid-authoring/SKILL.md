---
name: spid-authoring
description: Author SPID (Spell Perk Item Distributor) `_DISTR.ini` files — runtime, no-ESP distribution of spells, perks, items, shouts, keywords, outfits, factions, and AI packages to NPCs — using the bundled SPID grammar reference rather than invented syntax. Use when the user wants to write or fix a SPID `_DISTR` ini, distribute a spell/perk/item/keyword/outfit to NPCs, give all bandits or guards or a whole race or faction some form, add a keyword to NPCs, hand out gear or an ability filtered by gender/level/skill/chance, set an AI package on NPCs, make a distribution deterministic, or asks which SPID filters and traits target which NPCs (string/form/level/trait filters, the `F`/`M`/`U`/`T` trait codes, the deterministic `!` chance, the pipe-section syntax). Load this before writing any SPID line — the seven pipe sections, the `0x123~Plugin.esp` suffix-tilde FormID form, and which filter means what are non-obvious, and a wrong token silently distributes nothing at game load with no error.
---
