...
| 1b | SNAM subtype marker | just set `Subtype` — `housecarl_bulk_create`/`create_record` **auto-fill** the `SubtypeName` (SNAM) marker from it (Custom→`CUST`, Hello→`HELO`, Goodbye→`GBYE`…) and report it | the game buckets topics by this 4-char marker, so a topic with a `Subtype` but a **blank** marker (0000) is a **guaranteed load CTD**; only set `SubtypeName` by hand to override, or when editing a topic outside the create path (then `validate_dialogue` flags a blank one) |
