...
description: Look up Papyrus function signatures, parameter types, return types, flags, and documentation from bundled reference files when reading or writing .psc scripts. Bundled corpus covers vanilla Skyrim + SKSE + ~45 popular SKSE-plugin sources (PapyrusUtil, JContainers, MCMHelper, RaceMenu, PO3 Papyrus Extender, ConsoleUtil, and others). Use when the user is editing a Papyrus script, investigating a function call, looking up an SKSE plugin function, calling JContainers / PapyrusUtil / SkyUI / MCM scripting APIs, debugging a Papyrus compile error with unknown-identifier or type-mismatch errors, asking what a Papyrus function or event does, or checking a parameter type before authoring a script or event handler. Load this before answering any "what does this function do" or "what parameters does X take" question about Papyrus or SKSE-plugin scripting — if the function isn't in the bundle, this skill's bundled-or-warn flow surfaces an explicit "no reference for X" message so Claude never invents a signature.
---
