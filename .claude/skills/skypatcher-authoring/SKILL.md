...
description: Author SkyPatcher INI patches — runtime, no-ESP record edits that filter Bethesda records and set/add/remove their properties — using the bundled SkyPatcher grammar reference rather than invented syntax. Use when the user wants to write or fix a SkyPatcher patch or `.ini`, buff/nerf/rebalance weapons or armor, change NPC stats/factions/inventory, add or remove keywords, edit a leveled list or container contents, retune spells/enchantments/potions, unlevel encounter zones, tweak cell lighting or fog, disable a placed reference, or asks which filters or operations a SkyPatcher record type supports (filterByWeapons, filterByNpcs, attackDamage, keywordsToAdd, mgefsToAdd…) or which subfolder a SkyPatcher `.ini` belongs in. Load this before writing any SkyPatcher INI line — the per-record filter and operation names, the `Plugin.esp|FormID` vs EditorID rules, and subfolder placement are non-obvious, and a wrong name silently no-ops at game load with no error.
---
