...
**`w` is what the write tools will accept, not merely what the library exposes a setter for.** An identity field is marked `id`: `FormKey` on a record, and `ModKey` / `Master` on the `SkyrimMod` and `MasterReference` structs. The write pre-flight refuses every one of them outright, because a record's identity is not free-edit content — whether or not the library exposes a setter, and `MasterReference.Master` and a record's `FormKey` do expose one. The reference reports all of them as `"w":false,"id":true` and counts them out of the type's `writable/total`, so **an `id` field is never writable** and the read view and the write tools agree. Change a record's identity by creating or copying the record, never by setting the field.
