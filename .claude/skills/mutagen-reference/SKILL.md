...
| `list` | **`[N]` mid-path** to step into an element (`Effects[0].Data.Magnitude`); at the **leaf**, target the list field itself and use verb + `key` (the index) — **not** a leaf bracket | `Add`, `Remove`, `SetAtIndex`, `InsertAtIndex`, `ReplaceAll` (not `Set`) | To **add a modeled element** (a leveled-list entry, an effect), `Add` with `compose:{type:'<ElementType>', ...}`. For a coercible-element list (e.g. a list of FormLinks), `Add`/`ReplaceAll` take plain value(s). The three that place one element differ only in WHERE it goes: `Add` appends, `SetAtIndex` overwrites the element at `key`, `InsertAtIndex` inserts a new one AT `key` and shifts the rest right (`key` = the list's length appends). Reach for `InsertAtIndex` when the position matters — a CTDA `Or` flag chains a row to the row after it, so a new arm of an existing OR-group has to land inside the run, which an `Add` at the end cannot do. |
