...
- **`housecarl_apply` labels a malformed inline op `ops[i]` — the member you wrote.** A refusal on an inline op
  named `op[i]`, a member no houseCARL 2.0 tool declares, so the handle it gave you to fix pointed at nothing in
  the call. Both altitudes that can refuse one now spell `ops[i]`: the tool's own `from=` check, and the engine's
  `formid` and `field_path` checks. An op generated by the `bundle=` / `assignments=` copy zip is named by the pair
  and path it came from, as before.
