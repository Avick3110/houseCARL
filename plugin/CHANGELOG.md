...
- **A fresh `patch=` name that would shadow a plugin your order is not loading is now refused.** A new patch name
  was checked only against the mod folders houseCARL owns and the plugins the order loads, so `patch="My Cool
  Patch"` beside someone else's switched-off mod holding `My Cool Patch.esp` minted a second plugin of that name —
  two files that can never both be active — without saying anything. The name is now checked against every mod
  folder MO2 knows (enabled, switched off, or not yet listed), the overwrite folder and the game's `Data` folder,
  for `.esp`, `.esm` and `.esl`. The refusal is one sentence naming the mod folder it found and the file inside it,
  and it gives both readings: pass `patch=` a name no mod folder already uses, or enable that plugin and target it
  if it is the one you meant. The name is never auto-suffixed around the collision, so a suffixed patch can no
  longer hide the file you may have meant. Two things are unchanged: a plugin the order IS loading still gets the
  name auto-suffixed, and a patch houseCARL owns is not a shadow — `into=` extends that one. The check runs on a
  fresh write, never on an `into=` extend.
...
- **An `into=` that names no houseCARL patch, or names a folder houseCARL does not own, now tells you what to try.**
  ...
  same name. The name you pass `patch=` next is checked against that plugin — see the entry on a fresh patch name
  that would shadow a plugin the order is not loading.
