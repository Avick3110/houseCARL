...
- **The `mutagen-reference` skill is rebuilt against the 2.0 tool surface.** It named four tools that no longer
  ...
  so an edit composed from it called nothing. Addressing a field now names `housecarl_apply`'s `field_path` and
  ...
  rule, that a write at the sub-struct field is refused and the child is edited by its own FormID. The shards
