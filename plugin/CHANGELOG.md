...
houseCARL 2.0.2 fixes two answers 2.0.1 got wrong and closes two costs it measured. `housecarl_create` now reports every field from the file it wrote, and a created record the file does not hold says so, with create's own remedy; that supersedes 2.0.1's interim entry that said the values were not re-read. An auto-spill no longer fails when a scanner or indexer holds the file it is about to write, because the reservation is now the file itself; a `to_file=` write keeps its temp-then-move, so a failure never touches the caller's file. `format="json"` carries non-ASCII text as itself, and `max_chars` counts characters, not bytes, on every json render including a spilled file; characters above U+FFFF still escape, as the entry says. A `tree` or `delta` reads its bodies a plugin at a time, and a bulk `create` under many different parents reads each parent's plugin once; the measurements are in their entries. The entries below are in the order they landed.

- **A bulk `housecarl_create` under many different parents no longer re-reads a whole plugin per parent.** Each
...
- **`format="json"` carries non-ASCII text as itself, and `max_chars` counts characters on every json render.** A
  ...
  json and HTML safety require). Every json the server writes goes through the one encoder, the rows of a `to_file=`
  artifact included, so a name is spelled the same way inline and in the file. This supersedes the `\uXXXX` clause that closes the 2.0.1 UTF-8 entry below, which
...
- **`housecarl_place` and `housecarl_nif_set` no longer tell you to sort a new mod above a winner it already
  ...
  out-rank this one. Three winners are answered apart, because neither instruction fits them: a path won by
  MO2's overwrite folder, which sits above every mod in the VFS (move or delete that copy); a path won by a BSA
  or the game's Data folder, which any enabled mod's loose copy beats at any priority, so no sort is asked for
  on either lane; and a path the destination folder itself already provides, where the placement replaces its
  own earlier copy and keeps winning, so there is nothing to enable or sort.
...
- **`HOUSECARL_MAX_SCHEMA_DEPTH` publishes the tool schemas cut to a nesting depth.** Set it on the server entry
  ...
  every published schema is cut there and closed with the same node the recursion bound already closes with, under
  a clause of its own: nesting continues below and is accepted, and is not spelled out in this document, having
  been cut at that depth by this variable. `housecarl_create` and `housecarl_apply`
  ...
  a whole number of 4 or more stops the server's start with one sentence naming the variable: a schema cut below 4
  loses the parameters and parameter types every call is checked against, which would quietly change what a call
  gets back. A cut that cannot reach the depth it was given stops the start the same way, naming the tool, rather
  than publishing a schema over the cap.
...
- **A write's verify no longer says "re-read clean" about bytes it never parsed, and a read annotates every
...
- **The plugin's own `README.md` describes the 2.0 surface.** This is the file the installer copies to
  ...
  Mutagen is kept warm in memory, which the lazy-read design contradicts. The grammar, the predicate language, the runtime layers and the
...
- **The `mutagen-reference` skill is rebuilt against the 2.0 tool surface.** It named four tools that no longer
  ...
  to the successor rather than with the edit. Addressing a field now names `housecarl_apply`'s `field_path` and
  the `op` verbs each cardinality accepts, authoring a record is `housecarl_create`, reading one is
  `housecarl_records`, and taking a field's value from a plugin you name rather than from the load-order winner
  is `op='CopyFrom'` with `from_source`. The body is about a quarter shorter and names every file it ships under
  `references/`, each with the condition to open it, so the route from the body to a schema block no longer has
  an undeclared leg. Two things it taught that were wrong are gone: the Mutagen version it pinned, which nothing
  in the skill maintained — the reference regenerates with the library, and the warning for an absent type now
  says so without a number — and the claim that giving a parent a child record, or dropping one, has no home.
  `housecarl_create`'s `parent` and `housecarl_remove`'s `formids` both express it; what stays is the narrower
  rule, that a write at a sub-struct field holding an owned child record is refused and the child is edited by
  its own FormID. The shards
  themselves are unchanged: they are still generated by construction from the record library.
...
- **`housecarl_create` in place now refuses an editorid the target plugin already defines, instead of re-creating
  ...
  so instead of offering `replace=`. `replace=` is refused off the in-place lane, for the reason its own description gives. The `into=`
...
- **A write refusal labels the element in the member you wrote.** A refusal on an inline `housecarl_apply` op named
  ...
  `ops[i]` and `formids[i]`. Both altitudes that can refuse an inline op are covered: the tool's own `from=` check,
  and the engine's `formid` and `field_path` checks. An op generated by the `bundle=` / `assignments=` copy zip is
  named by the pair and path it came from, as before.
- **A new plugin whose filename would shadow one your order is not loading is now refused.** A new patch name was
  checked only against the mod folders houseCARL owns and the plugins the order loads, so `patch="My Cool Patch"`
  beside someone else's switched-off mod holding `My Cool Patch.esp` minted a second plugin of that name — two
  files that can never both be active — without saying anything. The exact filename a write will create is now
  looked for across every mod folder MO2 knows (enabled, switched off, or not yet listed), the overwrite folder and
  the game's `Data` folder. The refusal is one sentence naming where it found it — the mod folder, the overwrite
...
- **Every tool description now fits inside the 2,048 characters Claude Code shows, and the grammar that was
  ...
  and transport rules were being described in a part of the text nobody read. Twelve tools were over it —
...
- **`housecarl_copy` names the MO2 mod folder each source resolved from, and `housecarl_copy_npc_appearance` is
  ...
  sits beside the second. A named plugin that is ENABLED names its folder too — `Donor.esp (from the active load
  order, MO2 mod folder "TheDonorMod")` — because the placement needs it whether or not MO2 ticks the mod; only the
  bare `winner` pole is the whole order and names none. One that came out of
  MO2's overwrite or the game's `Data` folder is named as that layer rather than as a mod; a source read from a
  file outside all of them says it has no mod folder to pass on; and a real mod folder whose own name is `Data` or
  `overwrite` is named and then withdrawn in the same sentence, because those names are reserved and a placement
  handed one reads the layer instead of the mod. The 1.x standalone-appearance tool is deleted: the same job is `housecarl_copy` for the
...
- **An `into=` that names no houseCARL patch, or names a folder houseCARL does not own, now tells you what to try.**
  ...
  same name. The name you pass `patch=` next is checked against that plugin — see the entry on a new plugin whose
  filename would shadow one the order is not loading.
