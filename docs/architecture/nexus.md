...
## Pinned by
- `NexusFileCheckProbe` (ci probe `nexus-file-check-guard`) — the per-file currency verdicts, the withdrawn-over-retired order, the NoFileId and LatestOnly degrade, the nxm-only fall-through, the id#fileid parse, and the mod id grouping.
- `NexusGraphqlProbe` (`nexus-graphql-guard`) — the mutation and subscription refusal with no false refusal, and the bounded, literal raw-query rendering.
- `RenderClampProbe` (`render-clamp-guard`) — the surrogate-safe clamps and the entity-decode order in the description renderer.
