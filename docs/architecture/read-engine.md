...
- Bodies are gathered one walk per plugin (`BodyGather`, `WinnerBodies`, `BodyPrefetch`); the gather is never a second error path, so a plugin whose walk faulted falls back to the per-record fetch and raises the same fault, in the same words, from the same place in the caller's own loop.
