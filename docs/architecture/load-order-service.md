...
- `ILoadOrderHost`, in `src/housecarl-mcp/LoadOrderHost.cs` with the `AssetCapture` it returns, is how an area reaches the head members areas share. It carries the head members the area split assigned to it from the start (`Resolver`, `Assets`, `CaptureRoots()`, `CaptureAssets()`, `WriteGate`, `Rulebook`), plus any head member two areas take (`CapturePinAndAssets`, `CapturePinAndRoots`, `Types`), and each only once some area actually takes it through the door: today `Resolver`, `Assets`, `CaptureRoots()`, `CaptureAssets()`, `CapturePinAndAssets(afterPin)`, `CapturePinAndRoots(afterPin)`, `WriteGate`, `Types` and `Rulebook`. The head implements each member once, explicitly, next to what it wraps; rows relayed from other areas sit together in one block of the head.
...
- `CapturePinAndRoots(afterPin)` is the same hold with the four roots in place of the asset build, for the lanes that need the roots but no asset build (the read area's pole lanes and the errors and dialogue sweeps): the pin, `afterPin`, then the roots, so a profile switch cannot split the pin from the roots.
...
- Each area's own interface extends `ILoadOrderHost` and carries the rest: the head members only that area takes, plus rows relayed from other areas. The first is `IAssetHost`, in `src/housecarl-mcp/AssetLayers.cs`; the second is `ICheckHost`, in `src/housecarl-mcp/RecordChecks.cs`; the third is `IReadHost`, at the top of `src/housecarl-mcp/RecordReads.cs`. Those three areas are their own classes, `AssetLayers`, `RecordChecks` and `RecordReads`, each built by the head over itself; writes and output are still parts of the head's class. `IAssetHost` relays rows from writes and output, which are not classes yet. The one relay between two classes is `IReadHost.OpenSkyPatcherReplay`, which the head passes to `AssetLayers`: it takes the head's asset capture, so reads reaches it through the head rather than holding the assets instance. An area otherwise calls another class's statics directly, as checks and assets call `RecordReads.OpenDialogueFold` and `RecordReads.UnresolvedFormId`.
...
lanes, which pass it the area's test seam `AfterReadPinForGuard`, and the errors and dialogue sweeps) are one-liners, the class-parent cache
