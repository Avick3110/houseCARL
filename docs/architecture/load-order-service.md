...
- `CaptureRoots()` returns `Mo2Roots` (mods, data, overwrite and profile folders) from one `_gate` hold: it derives the roots first and throws whatever derivation throws, so an instance that will not resolve is the caller's error, not a blank root.
