...
/// <summary>
/// The catalog-FREE, framework-AGNOSTIC extractor for the SKSE config audit. It scans one config file's TEXT for
/// the two things that can be validated against the load order without knowing what any framework MEANS:
/// <list type="number">
...
/// </list>
///
/// <para><b>Pure + line-local.</b> Extraction is a HEURISTIC over token SHAPES, not a per-framework parse: a
/// hex+plugin token in a comment or a disabled block still surfaces (enabled/disabled semantics are not modelled —
/// the framing is "references this file declares", never "references the DLL will use"). No JSON/TOML object model
/// is built; references are line-local, so the scan is one regex pass per line. What a reference is FOR — a filter,
/// a swap target, an AV alias — is framework semantics and stays at the skill layer.</para>
///
...
/// unambiguously an EditorID, and validating every string would drown the signal. The verdict
...
public static class SkseConfigReferenceExtractor
{
    ...
    // separators / PARENTHESES / JSON-INI structure that would mark a token boundary. Two deliberate charset choices:
    // (1) apostrophe is ALLOWED — excluding it truncates real names mid-word at the ' (kryptopyr's / Sanguine's mods); a
    // leading TOML single-quote that rides in is stripped in BuildTokenRef. (2) parentheses are EXCLUDED — a config
    // comment embedding a token in prose ("will cast fireball (Skyrim.esm|0x5)") otherwise grabs the whole prose prefix
    // as the plugin name. Cost: a plugin literally named "Mod (v2).esp" is missed — rare, and the tradeoff for killing
    // prose false-positives. The hex side is up to 16 digits so an OVERFLOW token is CAPTURED (as UNPARSEABLE) not
    // silently missed. The alnum look-around stops a token gluing onto an adjacent identifier.
...
/// <see cref="Line"/> is the 1-based source line (0 for a path gate); <see cref="Unparseable"/> carries the reason a
/// shape-matched token could not be normalized — never a silent guess. The OK / PLUGIN MISSING / DANGLING verdict is
