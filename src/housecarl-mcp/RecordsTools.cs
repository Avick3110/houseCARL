...
public static class RecordsTools
{
    ...
    [McpServerTool(Name = ToolNames.Records, ReadOnly = true, Title = "Read records (the 2.0 read surface)"),
     Description(
         ...
         "SOURCE — source= is the subject, versus= the comparison reference pole.\n" +
    ...
    public static string Records(
        ...
        [Description("SOURCE decides whose version you read; this is the SUBJECT of the call. Omit or \"winner\" for the load-order winner (the default). A plugin filename (e.g. \"OldPatch.esp\") reads THAT plugin's version WHEREVER the plugin lives — active in your order, or sitting on disk unticked — you do not have to know which, and the response STATES which arm resolved (active, or out-of-load-order and from where); use {\"file\": \"X.esp\", \"mod\": \"<mod folder>\"} when two mods ship the same filename. A plugin found in neither place is refused naming both places searched. A record the named plugin does not touch is refused naming the plugins that DO touch it — never silently absent. An off-order file's content sits OUTSIDE the epoch fingerprint and the response says so. {\"overlay\": \"skypatcher\", \"state\": \"pre\"|\"post\"} reads around the SkyPatcher INI layer (post = after it replays; INI content sits OUTSIDE the epoch fingerprint and the response says so). \"previous_provider\" is a versus= value only — it is measured FROM the subject this parameter names.")]
        ...
        [Description("TRANSPORT: max rows to render (default 500). The TRUE total is always reported; page a scan in exact windows with offset=. DECLARED COST: the derived-selection forms (delta/tree/chain/info_order, and any walk) consume EVERY scan match — their censuses and artifacts cover the full selection, and limit= windows only the rendered rows — so on a big order the SCAN TERMS (types=/plugins=/where=) are the cost bound: narrow them.")]
            int limit = 500,
        [Description("TRANSPORT: skip the first N matches (exact windows: offset=0/500/1000…). Windows tile only WITHIN one epoch — if two pages' epochs differ the load order changed mid-pagination; re-run from offset=0, do not stitch the pages.")]
            int offset = 0,
        [Description("TRANSPORT: character ceiling on the RENDER. Never truncates the result — an over-ceiling result SPILLS in full to a server-side JSONL artifact (line 1 = manifest with the query echo, the row schema, and the epoch) and the response names the file. 0 = the server default (~80k).")]
            int max_chars = 0,
            ...
            string? to_file = null) => Guard.Tool(ToolNames.Records, () =>
    {
        ...
        string ListLane()
        {
            ...
            if (form == "identity")
            {
                ...
                if (wantFile)
                {
                    var (s, aerr) = Artifacts.WriteResolve(rows, epoch.Epoch, toFile!, "to_file", Echo());
                ...
                {
                    var path = ResultsStore.NextPath(ToolNames.Records, epoch.Epoch);
                    var (s, aerr) = Artifacts.WriteResolve(rows, epoch.Epoch, path, "ceiling", Echo());
            ...
            {
                var path = ResultsStore.NextPath(ToolNames.Records, epoch2?.Epoch ?? "none");
        ...
        string WalkLane(string[] ids, HousecarlCore.ArtifactDemand? demand, string? echoSrc)
        {
            ...
            if (reverseCarrier)
            {
                ...
                if (wantFile)
                {
                    var (sp, aerr) = Artifacts.WriteEffectChains(results, epochR?.Epoch, toFile!, "to_file", Echo());
                ...
                {
                    var path = ResultsStore.NextPath(ToolNames.Records, epochR?.Epoch ?? "none");
                    var (sp, aerr) = Artifacts.WriteEffectChains(results, epochR?.Epoch, path, "ceiling", Echo());
                    if (aerr is not null) ResultsStore.Release(path);
            ...
            if (form == "chain")
            {
                ...
                if (wantFile)
                {
                    var (s, aerr) = Artifacts.WriteChain(rows, wEpoch?.Epoch, toFile!, "to_file", Echo());
                ...
                if (spill is null && truncated)
                {
                    var path = ResultsStore.NextPath(ToolNames.Records, wEpoch?.Epoch ?? "none");
                    var (s, aerr) = Artifacts.WriteChain(rows, wEpoch?.Epoch, path, "ceiling", Echo());
                    if (aerr is not null) ResultsStore.Release(path);
                    rendered = Render(aerr is null ? SpillState.Spilled(s!, manifestOnly: false) : SpillState.WriteFailed(aerr), out _);
                }
                return rendered;
                             ...
                             OrderStamp? epoch, List<KeyValuePair<string, string>> echo)
        {
            ...
            if (wantFile)
            {
                var (s, aerr) = Artifacts.WriteDelta(rows, epoch?.Epoch, toFile!, "to_file", echo);
                if (aerr is not null) return json ? JsonWire.RenderError(aerr, epoch) : "error: " + aerr;
                spill = SpillState.Spilled(s!, manifestOnly: true);
            }
            ...
            if (spill is null && truncated)
            {
                var path = ResultsStore.NextPath(ToolNames.Records, epoch?.Epoch ?? "none");
                var (s, aerr) = Artifacts.WriteDelta(rows, epoch?.Epoch, path, "ceiling", echo);
                if (aerr is not null) ResultsStore.Release(path);
                rendered = Render(aerr is null ? SpillState.Spilled(s!, manifestOnly: false) : SpillState.WriteFailed(aerr), out _);
            }
            return rendered;
                            ...
                            OrderStamp? epoch, List<KeyValuePair<string, string>> echo)
        {
            ...
            if (wantFile)
            {
                var (s, aerr) = Artifacts.WriteTree(rows, epoch?.Epoch, toFile!, "to_file", echo);
                if (aerr is not null) return json ? JsonWire.RenderError(aerr, epoch) : "error: " + aerr;
                spill = SpillState.Spilled(s!, manifestOnly: true);
            }
            ...
            {
                var path = ResultsStore.NextPath(ToolNames.Records, epoch?.Epoch ?? "none");
                var (s, aerr) = Artifacts.WriteTree(rows, epoch?.Epoch, path, "ceiling", echo);
                if (aerr is not null) ResultsStore.Release(path);
                rendered = Render(aerr is null ? SpillState.Spilled(s!, manifestOnly: false) : SpillState.WriteFailed(aerr), out _);
            }
            return rendered;
                                 ...
                                 List<KeyValuePair<string, string>> echo)
        {
            ...
            if (wantFile)
            {
                var (s, aerr) = Artifacts.WriteInfoOrder(rows, epoch?.Epoch, toFile!, "to_file", echo);
                if (aerr is not null) return json ? JsonWire.RenderError(aerr, epoch) : "error: " + aerr;
                spill = SpillState.Spilled(s!, manifestOnly: true);
            }
            ...
            {
                var path = ResultsStore.NextPath(ToolNames.Records, epoch?.Epoch ?? "none");
                var (s, aerr) = Artifacts.WriteInfoOrder(rows, epoch?.Epoch, path, "ceiling", echo);
                if (aerr is not null) ResultsStore.Release(path);
                rendered = Render(aerr is null ? SpillState.Spilled(s!, manifestOnly: false) : SpillState.WriteFailed(aerr), out _);
            }
            return rendered;
