...
public static class CopyTools
{
    ...
    public static string Copy(
        ...
        [Description("REQUIRED. WHERE THE WALK STARTS is yours: the fields to walk from, e.g. ['HeadParts','HairColor','HeadTexture','WornArmor'] for an NPC's appearance. Each must be a RECORD LINK or a LIST OF RECORD LINKS, judged on the field's DECLARED type — so a field the source happens to carry none of is still that shape. A path that is not a field, or whose entries are structures rather than links (Factions, Perks, Items), is REFUSED BY NAME rather than quietly seeding nothing: for those use " + ToolNames.Apply + "'s bundle=/assignments= zip, which transplants each named field WHOLE onto the target — one CopyFrom per named field per target, replacing what the target had rather than merging into it. A seed the source leaves UNSET or EMPTY clears the target's, and says so — the result is the source's look, not a mixture of both.")]
        ...
        [Description("DESTINATION: copy onto this EXISTING record, 'XXXXXX:Plugin.esp' — an active record, or one in the patch itself when you pass into=; the seed fields are set on it pointing at the internalized copies. ONLY those link fields are written — everything else keeps the DESTINATION record's own value, and an NPC's INLINE face values (tint layers, face morphs and parts, TextureLighting, weight) are not links at all, so no seed can carry them and the destination's own face survives the copy. Copying a face onto an EXISTING NPC therefore owes a SECOND call, " + ToolNames.Apply + "'s copy zip: bundle=['TintLayers','FaceMorph','FaceParts','TextureLighting','HairColor','HeadTexture','Weight'] with assignments= mapping this record to the source NPC. (The clone destination, new_editorid=, is a whole-record duplicate and owes none of that.) Pass this OR new_editorid.")]
            string? target = null,
