...
static class RowProjection
{
    ...
    /// substructs, which is what reaches an Effect's Data.Magnitude and a condition arm's parameters.</summary>
    internal const int DefaultDepth = 4;

    /// <summary>Separator between a row's cells: a mark a value cannot be mistaken for.</summary>
    const string Sep = " | ";

    /// <summary>Fold every outcome's field list. A failed read passes through; a root that resolved to something
    /// that is NOT a list fails that record loud, because the form's whole answer is rows.</summary>
    ...
    /// fails the record. Judged on the root's OWN emitted line; an absent or unreadable root is left alone.</summary>
    ...
    /// <summary>The fold itself, over the emitted lines in their emission order. A row is keyed by its PATH rather
    /// than by contiguity, and a cell by its path too, so overlapping roots give one row and one of each cell.</summary>
    ...
    /// <summary>One row: the joined text the text render prints, with the folded leaves carried beside it.</summary>
    static FieldValue Row(string key, IReadOnlyList<FieldValue> kept)
    {
        var text = new List<string>(kept.Count);
        foreach (var f in kept)
            // A container summary whose own sub-fields are right there keeps only its type.
    ...
    /// <summary>A line the fold does not own, as it should leave the form. The engine's expansion-truncation remedy
    /// is written for the fields form, so on this form the remedy — and only the remedy — is restated.</summary>
    ...
    /// or the root itself when the caller already indexed one element — or null when the line is not under one.</summary>
    ...
    /// annotations the ordinary render appends.</summary>
    static string Cell(FieldValue f, string rowKey, bool trimToType)
    {
        ...
        // The trim took away the FormID the summary named, so its resolve_names identity goes with it.
