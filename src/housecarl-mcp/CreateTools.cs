...
public static class CreateTools
{
    ...
    public static string Create(
        ...
        [Description("LANE: base filename for the NEW patch this call writes (default 'Patch'); auto-suffixed if taken, so a prior patch is never overwritten — except a name another mod folder already holds as a plugin your order is not loading, which is refused rather than suffixed, naming that folder and file. Mutually exclusive with into= and in_place= — naming both lanes is refused, never silently ignored.")]
            string? patch = null,
