...
public static class CreateTools
{
    ...
    public static string Create(
        ...
        [Description("The records to author, all into one artifact: [{record_type, editorid, ops?, parent?, collection?, grid?}, …] — or \"@<absolute path>\" to read that SAME array from a JSON manifest file; the path must be ABSOLUTE (the server resolves relative paths against its OWN working directory, not yours), and the file is read at CALL time. One record is a set of one. For a nested one-shot, declare the parent BEFORE the children whose parent= names its editorid. A member the shape does not declare is refused BY NAME at its element, never silently dropped. Each new record's FormID is auto-allocated in the receiving plugin's own 0x800+ range and REPORTED BACK: that is how you reference the record afterwards — to make ANOTHER record point at it, call this tool or " + ToolNames.Apply + " again with that FormID as the value and the same lane (into='<this patch>', or in_place='<that plugin>' in place). The pre-flight refusals include an unknown or ambiguous record_type, a missing editorid, an illegal field op, a nested child with no resolvable parent, an ambiguous collection, a type that cannot be created (the bare abstract base 'Global'/'GameSetting' needs a concrete subtype; the refusal names them), and a DialogBranch with no Flags. What else is reported, never silently: creating dialogue lines reports VOICE coverage (a response with no .fuz plays SILENT in game — the audio is yours to provide) and RESULT-SCRIPT binding (a bound script that is unwired or uncompiled runs nothing); creating cells reports the world content houseCARL does NOT author (lighting, terrain, water, navmesh — Creation Kit work). Authoring dialogue also AUTO-FILLS the Creation Kit's own bookkeeping and reports each field it filled: the INFO's FavorLevel (CNAM) and Flags (ENAM), a DLVW's DNAM and ENAM, a DLBR's Category (TNAM), the DIAL's Priority (PNAM) and its SubtypeName (SNAM) marker derived from Subtype, and the QUST's NextAliasID (ANAM) plus each objective's Flags (FNAM) and each alias's Flags (FNAM) and each REFERENCE alias's VoiceTypes (VTCK). Every auto-fill is NON-OVERRIDE: a value you passed always wins. A DLBR's Flags (DNAM) is REQUIRED and never filled — a new DialogBranch that passes no Flags is refused, because vanilla carries both shapes deliberately and each wrong guess breaks the branch in game: pass TopLevel for a menu entry the player can pick, or 0 for a scripted Say() topic that must stay hidden. What a fill does NOT decide — which topic a generic greeting reaches (quest priority), the PNAM a re-listed INFO has to carry to keep its place, and the branch and INFO flags no fill sets for you — is in " + ReadSentences.DialogueDocUrl + ".")]
        ...
        [Description("Overwrite a record the in_place= target ALREADY defines under one of the editorids in records=: that record is re-created FRESH at its own FormID from this call's spec, and everything else it held is DISCARDED. Without it such a collision refuses the WHOLE call before anything is written, naming each record — an editorid a plugin you did not author already uses is far more likely a name you did not know was taken than a re-run of your own create. Only a collision an overwrite can settle: a name the target holds on a record of ANOTHER type, or on two records at once, or on an override it carries, is refused whatever this says. Meaningless without in_place=, and refused there rather than ignored: a new patch has nothing to collide with, and into= already re-creates its OWN record at a stable FormID.")]
            bool replace = false,
...
public sealed record CreateFieldOp
{
    ...
    [SchemaValues(SchemaVocabulary.CreateVerbs), JsonPropertyName("op"), Description(WriteVerbs.OnCreateRecital + ". SetAtIndex OVERWRITES the element at key=; InsertAtIndex inserts a new one AT key= and shifts the rest right (key = the list's length appends). On a [Flags] enum, Add sets one bit and Remove clears one, leaving the others untouched. There is no CopyFrom on this surface — a record that does not exist yet has no other version to copy a field from; copying a field from ANOTHER record is " + ToolNames.Apply + "'s CopyFrom.")]
