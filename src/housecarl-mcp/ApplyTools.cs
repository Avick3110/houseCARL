...
public static class ApplyTools
{
    ...
    public static string Apply(
        ...
        [Description("The edits, all into one artifact: [{formid, field_path, op?, value?, values?, key?, entries?, compose?, composes?, from?, from_source?}, …] — or \"@<absolute path>\" to read that SAME array from a JSON manifest file. One op is a set of one. An op member the shape does not declare is refused BY NAME at its element, never silently dropped. The manifest is how a big job is run: write the ops once, dry-run the file, then apply it — and re-run the same manifest to recover an interrupted write (overrides are idempotent). op='Remove' CLEARS a nullable field — a nullable substruct, a nullable polymorphic field — and is REFUSED BY NAME on a required one rather than writing an invented null. Which verb an op may use is decided by the field's CARDINALITY, and this pre-flight is the source of truth for it: read the cardinality off the schema before composing the op. from_source= on an op takes that field's value from the plugin you NAME rather than from the load-order winner — how an edit is based on an authored plugin when a generated one is winning the record.")]
