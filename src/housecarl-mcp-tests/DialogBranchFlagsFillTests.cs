...
/// filled to 0 for every category, and 0 is the one value that kills a player branch: Flags carries TopLevel, and a
/// Category=Player branch that is not top level never reaches the player's dialogue menu, so the branch and every
/// topic under it are dead (#693). The fill now follows the category — TopLevel for a player branch (what the CK's
/// own branch dialog ticks), 0 for anything else — and stays non-override either way.
///
...
public sealed class DialogBranchFlagsFillTests
{
    ...
    /// <summary>A Command branch (a bribe/intimidate speech challenge) is not published through the player's branch
    /// menu, so the fill materialises the subrecord and assumes no flag.</summary>
    [Fact]
    public void ACommandBranchWithNoFlagsIsFilledToZero()
    {
        using var w = new RecordsWorld();
        ...
        Assert.NotNull(flags);
        Assert.Equal(default(DialogBranch.Flag), flags);
        Assert.Contains(o.Created[0].Ops, op => op.Label.Contains("Flags (DNAM", StringComparison.OrdinalIgnoreCase));
    }
}
