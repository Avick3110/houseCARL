...
A provider that refuses the whole server at `tools/list` over JSON schema nesting depth: set `HOUSECARL_MAX_SCHEMA_DEPTH` on houseCARL's server entry to the depth that provider accepts (10, for the one that reports "exceeds the maximum nesting depth of 10 levels"). The published schemas are cut there and closed with a node saying nesting continues below with the same shape and is accepted; what the tools accept does not change. It applies to the server entry, so every model behind that entry gets the cut schemas — use a second entry to split them. Unset, the schemas publish in full, and a value that is not a whole number of 3 or more stops the server with one sentence naming the variable — below 3 a schema cannot carry its own parameters, which every call is checked against.
