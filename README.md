...
- Windows.
- .NET Runtime 9.0 and ASP.NET Core Runtime 9.0, from the [.NET 9 download page](https://dotnet.microsoft.com/download/dotnet/9.0). Both are required. The ASP.NET Core installer does not include the base runtime. The setup utility ships framework-dependent like the server, so it needs the base runtime to start at all — without it Windows says so and links the download page — and it checks for the ASP.NET Core one and names it if that is what is missing.
